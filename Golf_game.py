...
if st.button("開始計算"):
    ...
    st.download_button("📥 下載 Excel", data=excel_buffer.getvalue(),
        ...
        # --- 是否進入個人比分模式 ---
    st.markdown("---")
    st.subheader("⚔️ 是否要進入個人比分模式？")

    if st.button("➡️ 進入個人比分設定"):
        st.session_state.page = "personal"
        st.session_state.selected_players = selected_players   # ✅ 沿用剛剛的球員
        st.session_state.course_selected = course_selected     # ✅ 沿用剛剛的球場資料
    # ======================================================
    # 個人比分模式
    # ======================================================
    def personal_score_page():
        st.title("⛳ 個人比分模式 (沿用團體比賽球員)")
        ...
        st.write(f"已選球員: {', '.join(players_list)}")

        handicaps = {}
        bets = {}
        scores = {p: [] for p in players_list}

        st.subheader("⚙️ 讓桿 & 賭金設定")
        for p in players_list:
            col1, col2 = st.columns(2)
            with col1:
                handicaps[p] = st.number_input(f"{p} 讓桿 (前後各讓)", -18, 18, 0, key=f"hcp_{p}")
            with col2:
                bets[p] = st.number_input(f"{p} 每洞賭金", 10, 1000, 100, key=f"bet_{p}")
        ...
        for i in range(18):
            ...
            for j, p in enumerate(players_list):
                score = cols[j].number_input(f"{p} 桿數", 1, 15, par[i], key=f"{p}_hole_{i}")
                scores[p].append(score)

        if st.button("⚡ 計算個人比分"):
            st.success("✅ 個人比分計算完成 (這裡放入勝負/賭金邏輯)")
            st.write(scores)
